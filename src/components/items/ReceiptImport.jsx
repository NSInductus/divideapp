  ...
  return (
    <section className="receipt-import" aria-labelledby="receipt-title">
      <div>
        ...
        <p className="receipt-copy">Haz una foto o sube la cuenta. Revisarás los platos antes de añadirlos.</p>
      </div>
      ...
      {result && (
        <div className="receipt-review">
          ...
          {editableItems.map((item, index) => (
            <div className="receipt-item-row" key={index}>
              <input
                className="input"
                value={item.name}
                aria-label={`Nombre del plato ${index + 1}`}
              ...
              <input
                ...
                step="0.01"
                value={item.price}
                aria-label={`Precio de ${item.name || `plato ${index + 1}`}`}
                onChange={e => updateItem(index, "price", e.target.value)}
              />
              <button
                className="item-remove inline-remove"
                type="button"
                aria-label={`Eliminar ${item.name || "plato"}`}
                onClick={() => setEditableItems(items => items.filter((_, i) => i !== index))}
              >
                ×
              </button>
            </div>
          ...
          <button className="btn small" type="button" onClick={() => setEditableItems(items => [...items, emptyItem()])}>
            Añadir línea
          </button>
          <button className="btn primary" type="button" onClick={importItems}>
            Añadir {editableItems.length} platos
          </button>
        </div>
      )}
    </section>
